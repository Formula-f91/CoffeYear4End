...
//
// รวมทุก step (1-5) + Chart ไว้ในไฟล์เดียว
// ใช้ Inner Navigator pattern เหมือน Affective
// แก้ปัญหา provider ถูก dispose เมื่อ push route ใหม่
//
import 'dart:math';
  ...
  return Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(session?.cuppingName ?? "Descriptive Assessment",
...
// Sample card — แสดงตามจำนวน sample จริง
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        Expanded(flex: 2, child: Column(children: [
          ...
          Text(data.totalScore.toStringAsFixed(0),
...
// Bottom nav buttons
Widget _dBottomNav(BuildContext context, {
    ...
    child: Row(children: [
      ...
      Expanded(flex: 2, child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(
          backgroundColor: secondaryColor2, foregroundColor: Colors.white,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(0)),
          padding: const EdgeInsets.symmetric(vertical: 16)),
...
Widget _dDescriptorBox(List<String> items, void Function(String) onRemove) {
  const Color _blue = Color(0xFF1E52C6);
  return Container(
...
Widget _dNoteField(TextEditingController c, void Function(String) onChanged) =>
    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      TextField(controller: c, maxLines: 4, onChanged: onChanged,
        decoration: InputDecoration(
          hintText: 'Add notes...', hintStyle: TextStyle(color: Colors.grey.shade400, fontSize: 13),
...
// Add descriptor row button
Widget _dAddDescriptorRow(String label, VoidCallback onTap) =>
...
// Custom slider with balloon thumb
Widget _dSlider(BuildContext context, String label, double value, void Function(double) onChanged) {
  return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    SliderTheme(
      data: SliderTheme.of(context).copyWith(
        ...
        thumbShape: _BalloonSlider(thumbRadius: 10, thumbValue: value.toInt(), color: secondaryColor2),
        overlayColor: secondaryColor2.withOpacity(0.1),
        tickMarkShape: const RoundSliderTickMarkShape(tickMarkRadius: 0),
      ),
      child: Slider(value: value, min: 0, max: 15, divisions: 15, onChanged: onChanged),
    ),
    LayoutBuilder(builder: (ctx, constraints) {
      ...
      return SizedBox(height: 36, child: Stack(clipBehavior: Clip.none, children: [
        ...
        Positioned(left: pos(1.5) - 20, top: 8, width: 40,
            child: const Text("Low", textAlign: TextAlign.center,
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500))),
        Positioned(left: pos(5.0) - 25, top: 8, width: 50,
            child: const Text("Medium", textAlign: TextAlign.center,
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500))),
        Positioned(left: pos(8.5) - 20, top: 8, width: 40,
            child: const Text("High", textAlign: TextAlign.center,
...
class _Step1State extends State<_Step1> {
  ...
  @override
  Widget build(BuildContext context) {
    return Consumer<DescriptiveProvider>(builder: (_, p, __) {
      ...
      return Scaffold(
        ...
        bottomNavigationBar: _dBottomNav(context,
          onBack: () => Navigator.of(context, rootNavigator: true).pop(),
          onNext: () {
        ...
        body: Column(children: [
          ...
          Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(20),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _dHeaderCard(p), const SizedBox(height: 22),
              _dSampleCard(p), const SizedBox(height: 24),
              _dSlider(context, "Fragrance", d.fragrance, p.setFragrance),
              const SizedBox(height: 30),
              _dSlider(context, "Aroma", d.aroma, p.setAroma),
              const SizedBox(height: 32),
              _dAddDescriptorRow("Fragrance / Aroma Descriptors",
                  () => _showDescriptorSheet(p)),
              const SizedBox(height: 12),
              _dDescriptorBox(d.fragranceAromaDescriptors,
        ...
        body: Column(children: [
          ...
          Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(20),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 30),
              _dSlider(context, "Aftertaste", d.aftertaste, p.setAftertaste),
              const SizedBox(height: 32),
              _dAddDescriptorRow("Flavor / Aftertaste Descriptors",
                  () => _showFlavorSheet(p)),
              const SizedBox(height: 12),
              _dDescriptorBox(d.flavorAftertasteDescriptors,
                  (x) => p.setFlavorAftertasteDescriptors(
                      List.from(d.flavorAftertasteDescriptors)..remove(x))),
              const SizedBox(height: 24),
              _dAddDescriptorRow("Main Tastes (select up to 2)",
                  () => _showMainTastesSheet(p)),
              const SizedBox(height: 12),
              _dDescriptorBox(d.mainTastes,
                  (x) => p.setMainTastes(List.from(d.mainTastes)..remove(x))),
              const SizedBox(height: 24),
        ...
        body: Column(children: [
          ...
          Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(20),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _dHeaderCard(p), const SizedBox(height: 22),
              _dSampleCard(p), const SizedBox(height: 24),
              _dSlider(context, "Sweetness", d.sweetness, p.setSweetness),
              const SizedBox(height: 28),
        ...
        bottomNavigationBar: _dBottomNav(context,
          ...
          onNext: () {
            ...
            Navigator.push(context,
                MaterialPageRoute(builder: (_) => _DescriptiveChart(provider: p)));
          }),
        body: Column(children: [
          _dProgressBar(5),
          Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(20),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _dHeaderCard(p), const SizedBox(height: 22),
              _dSampleCard(p), const SizedBox(height: 24),
              _dSlider(context, "Mouthfeel", d.mouthfeel, p.setMouthfeel),
              const SizedBox(height: 32),
              _dAddDescriptorRow("Mouthfeel Descriptors",
                  () => _showMouthfeelSheet(p)),
              const SizedBox(height: 12),
              _dDescriptorBox(d.mouthfeelDescriptors,
...
// ─────────────────────────────────────────────────────────────────────────────
class _DescriptiveChart extends StatefulWidget {
  final DescriptiveProvider provider;
  const _DescriptiveChart({required this.provider});
  @override State<_DescriptiveChart> createState() => _DescriptiveChartState();
}

class _DescriptiveChartState extends State<_DescriptiveChart> {
      ...
      body: data == null
          ...
          : SingleChildScrollView(padding: const EdgeInsets.all(24),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const Text("Descriptive Assessment",
                ...
                Container(
                  ...
                  child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    ...
                    Text(data.totalScore.toStringAsFixed(1),
                        style: TextStyle(fontWeight: FontWeight.bold,
                            fontSize: 22, color: secondaryColor2)),
                  ]),
                ),
                const SizedBox(height: 24),
                ...
                if (data.fragranceAromaDescriptors.isNotEmpty) ...[
                  _descriptorSection("Fragrance / Aroma", data.fragranceAromaDescriptors),
                  const SizedBox(height: 16),
                ],
                if (data.flavorAftertasteDescriptors.isNotEmpty) ...[
                  _descriptorSection("Flavor / Aftertaste", data.flavorAftertasteDescriptors),
                  const SizedBox(height: 16),
                ],
  ...
  Widget _buildSampleCard(SessionModel? session, DescriptiveFormData data) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(flex: 3, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(session?.samples[_selectedSampleIndex].name ?? "Coffee",
                style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
            Text(session?.samples[_selectedSampleIndex].roastLevel ?? "",
                style: TextStyle(color: Colors.grey.shade500, fontSize: 13)),
          ])),
          Container(height: 36, width: 1, color: primaryColor2, margin: const EdgeInsets.symmetric(horizontal: 10)),
          Expanded(flex: 2, child: Column(children: [
          ...
          Expanded(flex: 2, child: Column(children: [
            ...
            Text(data.totalScore.toStringAsFixed(0),
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: secondaryColor2)),
          ])),
        ]),
        const SizedBox(height: 16),
        const Text("Select coffee", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        const SizedBox(height: 10),
        Wrap(spacing: 8, runSpacing: 8,
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          Text("${score.toStringAsFixed(0)} / 15",
      ...
      Wrap(spacing: 8, runSpacing: 6,
        children: items.map((d) => Container(
          ...
          decoration: BoxDecoration(
              ...
              border: Border.all(color: const Color(0xFF1E52C6).withOpacity(0.4))),
          child: Text(d, style: const TextStyle(fontSize: 12, color: Color(0xFF1E52C6))),
        )).toList()),
    ]);
  }
}
...
class _BalloonSlider extends SliderComponentShape {
  ...
  final Color color;
  const _BalloonSlider({required this.thumbRadius, required this.thumbValue, required this.color});
  ...
  void paint(PaintingContext context, Offset center, {
    ...
    canvas.drawRRect(RRect.fromRectAndRadius(
        Rect.fromCenter(center: bC, width: bW, height: bH), const Radius.circular(20)), paint);
    final path = Path()
      ..moveTo(center.dx - 5, bC.dy + bH / 2)
      ..lineTo(center.dx, bC.dy + bH / 2 + tH)
      ..lineTo(center.dx + 5, bC.dy + bH / 2)..close();
    canvas.drawPath(path, paint);
    final tp = TextPainter(
      text: TextSpan(text: thumbValue.toString(),
...
class _RadarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final gridPaint = Paint()
      ..color = Colors.grey.shade300..style = PaintingStyle.stroke..strokeWidth = 0.8;
    ...
    for (int i = 0; i < count; i++) {
      ...
      canvas.drawLine(center,
          Offset(center.dx + radius * cos(a), center.dy + radius * sin(a)), gridPaint);
    }

    final fillPaint = Paint()..color = secondaryColor2.withOpacity(0.2)..style = PaintingStyle.fill;
    final strokePaint = Paint()..color = secondaryColor2..style = PaintingStyle.stroke..strokeWidth = 2;
    final dotPaint = Paint()..color = secondaryColor2..style = PaintingStyle.fill;
